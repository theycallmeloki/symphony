defmodule SymphonyElixir.TestSupport do
  ...
  defp workflow_content(overrides) do
    config =
      Keyword.merge(
        [
          ...
          observability_run_journal_root: nil,
          server_port: nil,
          server_host: nil,
    ...
    observability_run_journal_enabled = Keyword.get(config, :observability_run_journal_enabled)
    observability_run_journal_root = Keyword.get(config, :observability_run_journal_root)
    ...
    sections =
      [
        ...
        observability_yaml(
          ...
          observability_run_journal_enabled,
          observability_run_journal_root
        ),
  ...
  defp observability_yaml(enabled, refresh_ms, render_interval_ms, run_journal_enabled, run_journal_root) do
    [
      "observability:",
      "  dashboard_enabled: #{yaml_value(enabled)}",
      "  refresh_ms: #{yaml_value(refresh_ms)}",
      "  render_interval_ms: #{yaml_value(render_interval_ms)}",
      "  run_journal_enabled: #{yaml_value(run_journal_enabled)}",
      run_journal_root && "  run_journal_root: #{yaml_value(run_journal_root)}"
    ]
    |> Enum.reject(&is_nil/1)
    |> Enum.join("\n")
  end
